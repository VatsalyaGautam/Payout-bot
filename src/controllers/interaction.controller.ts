...
export async function handleInteraction(interaction: Interaction): Promise<void> {
    if (interaction.isAutocomplete()) {
        ...
        console.warn(`[InteractionController] Unknown interaction type: ${interaction.type}`);
    }
}
...
async function handleAutocomplete(interaction: AutocompleteInteraction): Promise<void> {
    try {
        ...
        const filtered = payments
            .filter(p =>
                (p.id ?? '').toLowerCase().includes(focusedValue.toLowerCase()) ||
        ...
        handleError(`[InteractionController] Autocomplete error (${interaction.id})`, err);
...
async function handleChatInput(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    console.log(`[InteractionController] Received "${interaction.commandName}" (ID: ${interaction.id})`);
