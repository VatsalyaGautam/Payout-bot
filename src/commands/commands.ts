...
import { getAllPayments, upsertPayment, deletePayment } from '@/services/payment.service';
...
export async function handleCheckCommand(
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: `${paidInCurrentMonth}/${totalInCurrentMonth} paid this month` })
    .setTimestamp();
...
export async function handleUpdateCommand(
  ...
  if (!existing) throw new ValidationError(`Payment with ID \`${id}\` not found.`);

  const name = interaction.options.getString('name')?.trim() ?? existing.name;
  const dayOfMonth = interaction.options.getInteger('day-of-month') ?? existing.dayOfMonth;
  const amount = interaction.options.getNumber('payment_amount') ?? existing.amount;
  const currency =
    (interaction.options.getString('currency') as 'USD' | 'INR') ?? existing.currency;
  const notes = interaction.options.getString('notes')?.trim() ?? existing.notes;
  const autoDebit = interaction.options.getBoolean('auto-debit') ?? existing.autoDebit;
  ...
  await upsertPayment({ id, name, dayOfMonth, nextDue, amount, currency, notes, autoDebit });
