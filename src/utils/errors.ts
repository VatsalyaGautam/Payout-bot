...
export async function handleInteractionError(
    ...
    if (isValidation) {
        console.warn(
            `[${new Date().toISOString()}] [WARN] ${context} ${(error as Error).message}`
        );
    } else {
    ...
    let content =
        'Sorry, an internal error occurred while processing your command.';
...
// utils/errors.ts — add this helper
export function isPostgresError(err: unknown): err is { code: string; constraint?: string } {
  return (
    typeof err === 'object' &&
    err !== null &&
    'code' in err &&
