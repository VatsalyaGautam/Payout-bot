...
async function getChannel(): Promise<TextChannel | null> {
    try {
        ...
        handleError('Checker: Failed to fetch channel', err);
        return null;
    }
}
...
export async function checkPayments(): Promise<void> {
    ...
    if (payments.length === 0) {
        console.log(
            `[${new Date().toISOString()}] No payments found in database. Done.`
        );
        return;
    }
    ...
    for (const payment of payments) {
        try {
            ...
            if (days < -1) {
                console.warn(
                    `  [SKIP] Overdue (${days}d): ${payment.name} due ${nextDue}`
                );
                continue;
            }
    ...
    console.log(`[${new Date().toISOString()}] Finished checking all payments.`);
}
