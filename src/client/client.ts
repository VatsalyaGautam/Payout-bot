...
import { BOT_TOKEN, APPLICATION_ID } from '@/constants';
...
export async function registerCommands(guildId?: string): Promise<void> {
        ...
        new SlashCommandBuilder()
            ...
            .addBooleanOption((opt) =>
                opt
                    ...
                    .setDescription(
                        'Mark this payment as paid for the current month'
                    )
            )
            .toJSON(),
    ...
    try {
        if (guildId) {
            ...
            console.log(
                'Slash commands registered globally (may take up to 1h)'
            );
        }
    } catch (err: unknown) {
        throw err; // bootstrap will handle logging
    }
}
